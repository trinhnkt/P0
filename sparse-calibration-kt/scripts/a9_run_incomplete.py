#!/usr/bin/env python3
"""Run every incomplete A9 seed-42 job. Completeness = test-set row count, not file size."""
...
# Smaller XES first so one dataset can finish; then remaining Junyi jobs.
JOBS = [
    ("xes3g5m", "t50", "dkt", 42),
...
def main():
    ...
    for ds, level, model, seed in JOBS:
        ...
        if is_complete(ds, out):
            ...
            continue
        if out.exists():
            print(f"REMOVE truncated {out.name} rows={n_rows(out)} expected={TEST_N[ds]}", flush=True)
            out.unlink()
        cmd = [
            sys.executable, "-u",
        ...
        if not is_complete(ds, out):
            got = n_rows(out) if out.exists() else 0
            print(f"INCOMPLETE after run {out.name} rows={got}", flush=True)
            n_fail += 1
            continue
        n_ok += 1
        print(f"OK {out.name} rows={n_rows(out)}", flush=True)
    print(f"A9 incomplete queue done ok={n_ok} skip={n_skip} fail={n_fail}", flush=True)
    if n_fail:
        sys.exit(1)
