#!/usr/bin/env python3
"""Rerun only incomplete/missing A9 Junyi+XES seed-42 jobs."""
...
# Junyi only in this project. XES jobs are not queued here.
JOBS = [
    ("junyi", "t500", "dkt", 42),
    ("junyi", "t100", "dkt", 42),
    ("junyi", "t50", "dkt", 42),
...
def main():
    ...
    for ds, level, model, seed in JOBS:
        ...
        batch = 32 if model == "simplekt" else 64
        ...
        if not is_complete(ds, out):
            ...
            continue
        n_ok += 1
    print(f"A9 missing queue done ok={n_ok} skip={n_skip} fail={n_fail}", flush=True)
    if n_fail:
        sys.exit(1)
